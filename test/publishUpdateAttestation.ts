...
did.createAttestation(claimBundleHash).then(attestation1 => {

DID.publishAttestation(did.getMethodSpecificIdentifier(), attestation1).then((value) => {
    console.log('attestation 1 published')
    setTimeout(() => {
    did.createAttestation(claimBundleHash, 0).then(attestation2 => {
      DID.publishAttestation(did.getMethodSpecificIdentifier(), attestation2).then((value) => {
